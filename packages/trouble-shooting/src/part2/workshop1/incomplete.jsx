...
class MatchCard extends Component {
  render() {
    const {
      card: { name, image, age, company, education },
    } = this.props;
...
class MatchList extends Component {
  render() {
    ...
    return <div style={pageStyles.matchLogRoot}>{listItems}</div>;
  }
}
  ...
  render() {
    return (
      <div style={pageStyles.matchControllerRoot}>
        <button style={pageStyles.matchButton} onClick={this.props.next}>
          skip
        </button>
        &nbsp;
        <button style={pageStyles.matchButton} onClick={this.props.like}>
          Like
        </button>
      </div>
    );
